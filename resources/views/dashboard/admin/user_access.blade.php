...
<section class="w-full h-full">
    <div class="min-w-full">
        ...
        <div class="w-full flex flex-col md:flex-row items-start md:items-center font-semibold justify-between mb-4">
            <h1 class="text-3xl font-semibold mb-4">Users Access</h1>
            ...
            <form method="GET" action="" class="md:mb-0 mb-4 w-full md:max-w-[350px] flex flex-row items-center overflow-hidden rounded-md border border-gray-300">
                <select id="select" name="select" class="bg-gray-50 w-full border border-gray-300 text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block p-2.5">
                    ...
                    @foreach ($roles as $row)
                        <option {{ ($row->name == $select ) ? "selected" : "" }} value="{{ $row->name }}">{{ $row->name }}</option>
                    @endforeach
                </select>
                ...
                <tbody>
                    @foreach ($users as $rows => $row)
                        <tr class="bg-white border-b dark:bg-gray-800 hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                @php $role_user = $row->roles->first()->name ?? "-"; @endphp

                                @if ($role_user == "pimpinan")
                                    <div class="px-5 py-2 font-semibold rounded-full w-fit bg-red-100 text-red-600">
                                        {{ ucwords(strtolower($role_user)) }}
                                    </div>
                                @elseif($role_user == "admin")
                                    <div class="px-5 py-2 font-semibold rounded-full w-fit bg-blue-100 text-primary">
                                        {{ ucwords(strtolower($role_user)) }}
                                    </div>
                                @else
                                    <div class="px-5 py-2 font-semibold rounded-full w-fit bg-green-100 text-green-500">
                                        {{ ucwords(strtolower($role_user)) }}
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <div class="gap-2 grid grid-rows-1 grid-flow-col w-fit">
                                    ...
                                    <button data-id="{{ $row->id }}" class="update-role-user px-1.5 py-1.5 bg-primary text-white w-fit rounded-md">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
    ...
    {{-- modal update hak akses  --}}
    <div id="modal-user-access" tabindex="-1" class="hidden fixed inset-0 bg-gray-500 bg-opacity-75 z-50 px-4">
        <div class="flex justify-center items-center w-full h-full">
            <div class="modal-user-access-close absolute w-full h-full"></div>
            <div class="relative w-full max-w-md max-h-full">
                <!-- Modal content -->
                <div id="input-role-user" class="relative bg-white rounded-lg shadow">
                    <div class="flex items-center justify-between py-4 px-6 border-b rounded-t">
                        <h3 class="text-lg font-medium text-gray-900">Hak Akses</h3>
                        <button type="button" class="modal-user-access-close text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
                    ...
                    <div class="px-6 py-4 ">
                        <p class="text-sm text-gray-900 mb-4">
                            Mengubah hak akses dapat mempengaruhi peran yang user miliki dalam mengelola data di website ini.
                        </p>
                        <div class="mb-0">
                            <label for="hak-akses" class="block mb-2 text-sm font-medium text-gray-900">Hak Akses</label>
                            <input type="hidden" id="id-hak-akses-user" name="">
                            <select id="hak-akses-user" name="hak_akses" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"></select>
                        </div>
                    </div>
                    <div class="flex items-center justify-end py-3 px-6 space-x-2 border-t border-gray-200 rounded-b">
                        <div class="modal-user-access-close hover:bg-gray-500 text-white cursor-pointer bg-gray-400 font-medium rounded-md text-base px-6 py-2.5 text-center">Batal</div>
                        <button type="button" id="button-update-hak-akses" class="modal-user-access-close text-white bg-primary hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-md text-base px-6 py-2.5 text-center ">Simpan</button>
                    </div>
                </div>
            </div>
        </div>
...
<script src="{{ asset('/js/admin/update_role_user.js') }}"></script>
@endsection